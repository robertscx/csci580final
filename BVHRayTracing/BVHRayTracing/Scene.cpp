...
void Scene::sampleLight(Intersection& pos, float& pdf) const
{
    ...
    for (uint32_t i = 0; i < objects.size(); i++) {
        // find emission object in the scene as light source, summing theri area.
        if (objects[i]->hasEmit()) {
            area_sum += objects[i]->getArea();
        }
    }

    float randomA = get_random_float() * area_sum;
    area_sum = 0;
    // get the light source when area_sum >= randomA
    for (uint32_t i = 0; i < objects.size(); i++) {
        if (objects[i]->hasEmit()) {
            area_sum += objects[i]->getArea();
            if (randomA <= area_sum) {
...
Vector3f Scene::castRay(const Ray& ray, int depth) const
{
    ...
    if (!hitObj.happened) return {}; // if intersect with object
